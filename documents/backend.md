...
│   └── aiVision.js        # recognizedItems 陣列（草稿）+ analyzeImage／updateItem／removeItem／discard／confirmSelected；confirmSelected 是唯一會寫入 Supabase 的入口，且只呼叫 useFoodStore().addFood()，不直接碰 foodService／repository
├── services/
│   ├── foodService.js         # store 與 repository 之間的欄位轉換層（詳細的表結構/RLS 見 documents/database.md）
│   ├── shoppingListService.js # 同上
│   ├── recipeService.js       # 同上
│   ├── authService.js         # 同上
│   └── aiVisionService.js     # recognizeFoods(file)：把圖片轉成縮小過的 base64 JPEG，呼叫 Supabase Edge Function `analyze-food-image`，把回傳的 items 轉成帶 tempId／emoji／selected 的草稿物件（見 documents/database.md、`.claude/skills/ai-vision/SKILL.md`）
├── views/
...
│   ├── ScanFood.vue       # isAnalyzing／isSubmitting／hasAnalyzed／errorMessage；analyzeImage() 失敗與 confirmSelected() 失敗各自對應獨立的錯誤文案
...
│   ├── ImageCapture.vue     # 純輸入元件，選好檔案只 emit('select', file)，不呼叫任何 store／service
│   ├── AiRecognitionResult.vue  # 接收 items（草稿陣列）／isSubmitting props，勾選／改名／改分類／移除都用 emit（toggle／update-name／update-category／remove）往上丟，自己不改資料、也不呼叫 store
...
- **AI 辨識結果是草稿，使用者確認才寫入**：`stores/aiVision.js` 的 `recognizedItems` 只存在記憶體裡，`analyzeImage()`／`updateItem()`／`removeItem()`／`discard()` 都只改這個本地陣列，完全不碰 Supabase；只有使用者在 `ScanFood.vue` 按下「確認加入冰箱」觸發的 `confirmSelected()` 才會逐筆呼叫 `useFoodStore().addFood()` 寫入資料庫（用 `Promise.allSettled` 讓部分失敗不影響其他項目成功寫入，失敗的項目留在草稿裡讓使用者重試，成功的則從草稿移除）——這是 `.claude/skills/ai-vision/SKILL.md` 規定的硬性流程：AI 只是協助輸入資料的助手，辨識結果永遠是待確認的草稿，不是可信來源。`aiVisionService.recognizeFoods()` 呼叫的是 Supabase Edge Function（`supabase.functions.invoke('analyze-food-image')`），不是 repository／service 的一般資料表 CRUD，圖片本身也不會被上傳儲存（詳見 `documents/database.md`）
